var  area = {
  ...
  "세종": [
    "조치원읍", "금남면", "부강면", "연서면", "연동면", "소정면",
    "전의면", "전동면", "고운동", "아름동", "종촌동", "도담동"
  ],
  "경기": [
    "수원시", "성남시", "의정부시", "안양시", "부천시", "광명시", "평택시",
    "동두천시", "안산시", "고양시", "과천시", "의왕시", "구리시", "남양주시",
...
function addr_select_draw(id){
    var list ='';
    list += '<option value="">지역</option> ';
    list += '<option value="">전체</option> ';
    for (var addr in area) {
        list += '<option value="' + addr + '">' + addr + '</option>';
    }
    $('#'+id).html('');
    $('#'+id).append(list);
}
